...
FRC_ActionBar.new = function(args)
	checkoptions.callee = 'FRC_ActionBar.new';
	...
	menuItems.xHidden = -bgRect.width * 0.5;
...
function FRC_ActionBar.onUnrelatedTouch(event)
	if (not activeMenu) then return; end
	activeMenu:hide();
end

FRC_ActionBar:addEventListener('unrelatedTouch', FRC_ActionBar.onUnrelatedTouch);

return FRC_ActionBar;
